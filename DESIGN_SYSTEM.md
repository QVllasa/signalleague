# SignalLeague Design System — Cyberpunk / Glitch
...
**Chromatic Aberration** (via CSS on `.cyber-glitch`):
...
3. **Noise Texture**: Subtle CSS noise filter or SVG noise overlay at 5-10% opacity
...
- Hover: translateY(-1px), border becomes accent, neon glow appears
```
...
- Use for: Blog cards, FAQ items, code blocks
```
...
- Use for: Featured content, hero HUD panels
```
...
8. **Typing/Typewriter Effect**: Consider on subtitle or style as if mid-type (trailing cursor)
...
- Stats: 2x2 grid (mobile) -> 4-column with vertical borders (desktop)
...
**Contrast**: All text meets WCAG AA (accent green on dark bg = 7.5:1 ratio)
